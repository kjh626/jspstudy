...
public class HelloServlet extends HttpServlet {
	...
	*/
	...
	/*
		4. doGet()
			...
			2) GET 방식의 요청의 예시
				(1) <a href="http://localhost:9090/01_Servlet/HelloServlet">   => a링크의 클릭은 100% GET방식이다.
	...
	*/
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 서버 측에서 데이터 (html태그를 텍스트로) 보내주겠다(writer).  데이터 보내는 출력스트림을 만들어서 (글자를) 보낸 거다.
		// 클라이언트쪽으로 보내는 최종 정보 = Served at: /01_Servlet
	...
	/*
		5. doPost()
			...
			2) Post 방식의 요청의 예시
				(1) <form method="POST" action="http://localhost:9090/01_Servlet/HelloServlet">  이렇게 되면 doPost가 받는다.
				(2) $.ajax({
				type: 'POST',
				url: 'http://localhost:9090/01_Servlet/HelloServlet',
				});
	*/
	...
	// doPost는 값 받아와서 doGet으로 다 전해준다. => 모든 데이터 처리는 doGet이 처리한다고 볼 수 있다.
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// POST 메소드를 통해서 넘어온 정보를 모두 doGet() 메소드에 넘긴다.
		doGet(request, response);
	}
	// init(), service()는 이제 안 적을 거임. doGet, doPost만 만들거다. 서비스 없으면 doGet() 또는 doPost() 메소드가 자동으로 호출된다.
	// 작업은 doGet에 다 적어줄 거임.
